...
#include "helpers/protodetect_storage.h"
#include "views/protodetect_main_view.h"
...
static void draw_callback(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(canvas, 0, 25 + (app->current_menu * 10), ">");
}
            ...
            if(event.type == EventTypeKey) {
                if(event.input.type == InputTypeShort) {
                    switch(event.input.key) {
                        case InputKeyUp:
                            if(app->current_menu > 0) app->current_menu--;
                            break;
                        case InputKeyDown:
                            if(app->current_menu < 2) app->current_menu++;
                            break;
                        case InputKeyOk:
                            // TODO: Menüpunkt ausführen
                            break;
                        case InputKeyBack:
